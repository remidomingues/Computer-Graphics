...
void PixelShader(Pixel& p)
{
	...
	if (p.zinv > depthBuffer[y][x])
	{
		...
		float distance = GetDistance(p.pos3d, lightPos);
		float div = 4 * M_PI * distance * distance;
		float max = fmax(glm::dot(r, currentNormal), 0);
		vec3 directLight(lightPower.x * max / div,
			lightPower.y * max / div,
			lightPower.z * max / div);
		vec3 illumination = (directLight + indirectLightPowerPerArea) * currentReflectance;
...
void Interpolate(Pixel a, Pixel b, vector<Pixel>& result) {
	...
	vec3 positionStep = (b.pos3d - a.pos3d) / div;
